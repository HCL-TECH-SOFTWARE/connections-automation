...
echo "Setting up host name in environments/examples/vagrant/vagrant_connections_inventory to ${hostname}"
sed -i '' "s/my-local-hostname/${hostname}/" environments/examples/vagrant/vagrant_connections_inventory
...
then
  ...
  echo "Port ${internal_download_port} is not open. If that is used in environments/examples/vagrant/vagrant_connections_inventory then your package download will fail. Will continue executing Vagrant in 10 seconds."
  sleep 10
fi
